...
## 4a. Hosted Web App Data Flow

The optional hosted web app (Vercel) behaves slightly differently from the CLI, and we want to be precise:

- **Ranking is done server-side, with no AI.** When you search, your ZIP code, budget, and care-needs selections are sent to the DearNana serverless function to fetch CMS data and rank facilities. The ZIP code is resolved to a location **offline** — the web app contacts no geocoding service — and your inputs are processed **transiently for that one request and are not stored, logged, or sold.** No AI is involved in this step.
- **Your Anthropic API key never reaches our servers.** The optional AI report is generated by your **browser**, which sends the prompt and your key **directly to Anthropic** over HTTPS. Our server never receives, stores, or logs the key. The key is held only in your browser tab (cleared when you close it or click "Forget key").
- A strict Content-Security-Policy limits the page so that outbound requests can only reach our own server and `api.anthropic.com` — your key cannot be exfiltrated to any other host.
- **Residual note:** because the key is used in the browser, anyone with full control of your device/browser could read it. For maximum safety, use a separate, revocable key, or use the CLI with local Ollama where nothing leaves your machine.
