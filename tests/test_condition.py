...
class TestParseCondition:
    ...
    def test_no_provider_uses_keywords(self, monkeypatch):
    ...
    def test_ollama_provider_success(self, monkeypatch, mocker):
        monkeypatch.setenv("DEARNANA_LLM_PROVIDER", "ollama")
        profile_json = NeedsProfile(
            needs=[Need(category="fall_risk", weight=1.0)], summary="fall risk"
        ).model_dump_json()
        mock_resp = mocker.Mock()
        mock_resp.json.return_value = {"message": {"content": profile_json}}
        mock_resp.raise_for_status.return_value = None
        mocker.patch("dearnana.llm.httpx.post", return_value=mock_resp)

        profile = parse_condition("she has fallen twice")
        assert profile.source == "ai"
        assert "fall_risk" in profile.categories()

    def test_ollama_down_falls_back_to_keywords(self, monkeypatch, mocker):
        monkeypatch.setenv("DEARNANA_LLM_PROVIDER", "ollama")
        import httpx

        mocker.patch(
            "dearnana.llm.httpx.post", side_effect=httpx.ConnectError("refused")
        )
        profile = parse_condition("she has fallen twice")
        assert profile.source == "keywords"
        assert "fall_risk" in profile.categories()
