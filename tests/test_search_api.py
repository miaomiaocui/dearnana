...
# --- rate limiting (no network: Upstash globals + _redis_cmd mocked) ---

def test_rate_limit_noop_without_upstash(monkeypatch):
    monkeypatch.setattr(search, "UPSTASH_URL", None)
    monkeypatch.setattr(search, "UPSTASH_TOKEN", None)
    assert search.is_allowed("1.2.3.4") is True


def test_rate_limit_blocks_over_threshold(monkeypatch):
    monkeypatch.setattr(search, "UPSTASH_URL", "http://x")
    monkeypatch.setattr(search, "UPSTASH_TOKEN", "t")
    monkeypatch.setattr(search, "RATE_LIMIT", 3)
    n = {"c": 0}
    ...
    monkeypatch.setattr(search, "_redis_cmd", fake_cmd)
    assert [search.is_allowed("ip") for _ in range(5)] == [True, True, True, False, False]


def test_rate_limit_fails_open_on_error(monkeypatch):
    monkeypatch.setattr(search, "UPSTASH_URL", "http://x")
    monkeypatch.setattr(search, "UPSTASH_TOKEN", "t")
    ...
    monkeypatch.setattr(search, "_redis_cmd", boom)
    assert search.is_allowed("ip") is True  # never block on limiter failure
