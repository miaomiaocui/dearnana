...
score. Parsing uses a small LLM call (Anthropic API or local Ollama, see
dearnana.llm) when a provider is available, with a keyword matcher as
fallback so personalization always works offline.
"""
...
class Need(BaseModel):
    ...
    weight: float = Field(default=1.0, ge=0.0, le=1.0)  # relative importance, 0-1
    rationale: str = ""
...
class NeedsProfile(BaseModel):
    ...
    summary: str = ""
    source: Literal["ai", "keywords", "interactive", "none"] = "none"
...
def _parse_with_llm(text: str) -> NeedsProfile | None:
    """Single small LLM call returning a validated NeedsProfile.

    Returns None when no provider is configured or on any provider failure
    so the caller can fall back to keyword parsing.
    """
    provider = get_provider()
    if provider is None:
        return None
    profile = provider.parse(_PARSER_PROMPT.format(text=text), NeedsProfile)
    if profile is None:
        return None
    profile.source = "ai"
    return profile
...
def parse_condition(text: str, use_llm: bool = True) -> NeedsProfile:
    ...
    Uses the configured LLM provider (Anthropic API or local Ollama) when
    use_llm is True and one is available; falls back to keyword matching
    otherwise (or on any provider failure). Never raises.
    """
    if not text or not text.strip():
        return NeedsProfile(source="none")

    if use_llm:
        profile = _parse_with_llm(text)
        # Trust the provider's answer when it succeeds — even an empty profile.
        # The model can read negation ("does NOT have dementia"); the keyword
        # matcher cannot, so only fall back to keywords when no provider is
        # configured or the call/validation fails (profile is None).
