"""Single Claude API call for personalized nursing home recommendation."""

import os

import anthropic

from dearnana.config import LLM_MAX_TOKENS, LLM_MODEL, LLM_TEMPERATURE
...
def generate_recommendation(
    ...
    """Generate personalized recommendation using Claude. Single API call.

    Falls back to a data-only summary if no API key is set or the API
    call fails.
    """
    ...
    try:
        ...
        return _generate_fallback(
            ranked,
            notice="AI recommendation unavailable (API error) — showing data-only summary.",
        )

    text = next((b.text for b in message.content if b.type == "text"), "")
    return text or _generate_fallback(ranked)
...
def _generate_fallback(ranked: list[RankedFacility], notice: str = "") -> str:
    ...
    lines = [
        notice or "(Set ANTHROPIC_API_KEY for personalized AI recommendations)",
