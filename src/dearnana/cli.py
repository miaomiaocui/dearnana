...
def main(address: str, budget: float, condition: str, radius: float, top_n: int, no_ai: bool):
    ...
    if not no_ai and provider == "anthropic" and not os.environ.get("ANTHROPIC_API_KEY"):
        click.echo(
            ...
            "  get the AI-written recommendation report. To enable it, either get a key\n"
            "  at https://console.anthropic.com and set ANTHROPIC_API_KEY (or put it in\n"
            "  a .env file), or run a local model: install Ollama (https://ollama.com)\n"
            "  and set DEARNANA_LLM_PROVIDER=ollama.\n"
        )
